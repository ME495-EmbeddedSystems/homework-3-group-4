"""
Motion planning interface for moveit API.

This class incorporates MotionPlanner, RobotState and PlanningScene classes,
offering an interface for a user to interact with the moveit API.
"""
...
class MotionPlanningInterface:
    """
    Main class interacting with moveit API.

    A class that consolidates MotionPlanner, RobotState and PlanningScene
    classes in one class that is able to be utilized by any user node
    for any planning and execution task.
    """
    ...
    def __init__(self, node: Node):
        ...
        Initializes MotionPlanningInterface with an instance of the ROS node
        passed to it as an argumet
        """
        self.node = node
    ...
    async def plan_path(
        ...
        """
        Plan path using MotionPlanner class.
        ...
        :param named_configuration: named configuration inputfor path planning
        ...
        if goal_joints is not None:
            ...
            self.path = await self.motion_planner.plan_cartesian_path(
                waypoints)
