...
class MotionPlanningInterface:
    ...
    def __init__(self, ros_node: Node):
        ...
        self.robot_state = CustomRobotState(ros_node = self.node)
        self.motion_planner = MotionPlanner(ros_node = self.node, robot_state = self.robot_state)
        ...
        async def plan_path(
            ...
            if goal_joints is not None:
                ...
                self.path = await self.motion_planner.plan_pose_to_pose(goal_pose)
            elif waypoints is not None:
                self.path = await self.motion_planner.plan_cartesian_path(waypoints)
        

        async def exec_path(path: MotionPlanRequest = None):
            if path is None:
                # Error message
                result = 0
            else:
